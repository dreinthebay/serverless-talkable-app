...
rewardController
  .post('/', async (req, res, next) => {
    ...
    const reward = await Reward.create(JSON.parse(req.body.payload)) //--Mongo code --
    const kafkaPayload = await [{ topic: 'talkable-development', messages: reward, partition: 0 }];
